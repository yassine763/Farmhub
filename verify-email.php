<?php
// verify-email.php
require_once 'config/db.php';

$error = "";
$success = "";
$email = $_GET['email'] ?? "";

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $code = trim($_POST['code']);
    $email = trim($_POST['email']);

    if (!empty($code) && !empty($email)) {
        $stmt = $pdo->prepare("SELECT id FROM users WHERE email = ? AND verification_code = ?");
        $stmt->execute([$email, $code]);
        $user = $stmt->fetch();

        if ($user) {
            // Marquer comme vérifié
            $update = $pdo->prepare("UPDATE users SET is_verified = 1, verification_code = NULL WHERE id = ?");
            $update->execute([$user['id']]);

            $success = "Votre email a été vérifié avec succès !";
            header("Refresh: 2; URL=login.php");
        } else {
            $error = "Code de vérification incorrect.";
        }
    } else {
        $error = "Veuillez entrer le code.";
    }
}
?>
<!DOCTYPE html>
<html lang="fr">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Vérification Email - FarmHub</title>
    <link rel="stylesheet" href="css/style.css">
</head>
<body class="auth-wrapper">
    <div class="auth-card">
        <h2>Vérifiez votre email</h2>
        <p>Un code de bienvenue a été envoyé à <strong><?php echo htmlspecialchars($email); ?></strong>.</p>

        <?php if ($error): ?>
            <div class="error-msg"><?php echo $error; ?></div>
        <?php endif; ?>
        
        <?php if ($success): ?>
            <div class="success-msg"><?php echo $success; ?></div>
        <?php endif; ?>

        <form method="POST">
            <input type="hidden" name="email" value="<?php echo htmlspecialchars($email); ?>">
            <div class="form-group">
                <label>Code de vérification</label>
                <input type="text" name="code" placeholder="123456" maxlength="6" style="text-align: center; letter-spacing: 5px; font-size: 1.5rem;" required>
            </div>
            <button type="submit" class="btn btn-primary btn-full">Vérifier mon compte</button>
        </form>
    </div>
</body>
</html>
